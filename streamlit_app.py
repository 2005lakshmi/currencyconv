...
st.title(":blue[Currency] :green[Converter]",)
...
def function1():
    ...
    qty = st.number_input("enter the amount",0)
    fromC = st.selectbox("choose its currency:",currencies,index=66)
    li = fromC.split(" ")
    fromC = li[0]
    toC = st.selectbox("choose currency to be coverted:",currencies,index=66)
    li  = toC.split(" ")
    toC = li[0]
    fromC = fromC.upper()
    toC = toC.upper()
    ...
    file1.close()
    st.success(f"{qty} of currency {fromC} amounts to {amount} of currency {toC} today")
